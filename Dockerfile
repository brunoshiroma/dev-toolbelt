...
RUN cd /tools/zlib && CC=musl-gcc ./configure --static && make && make install
...
#
#RUN native-image --no-fallback --static --libc=musl -jar build/libs/devtoolbelt.jar -Dspring.native.remove-yaml-support=true -H:IncludeResources='templates/.*' -H:IncludeResources='static/.*' -H:IncludeResources='application.*properties\$' -H:Log=registerResource:verbose